...
export default function EditQuotePage() {
  ...
  useEffect(() => {
    ...
    // 编辑模式：从 URL 读取 id 并加载报价数据
    const urlId = new URLSearchParams(window.location.search).get('id');
    if (urlId) {
      setEditingQuoteId(Number(urlId));
      setIsEditingMode(true);
      fetch('/api/quotes?userId=1&id=' + urlId)
        .then(r => r.json())
        .then(async (result) => {
          if (result.success && result.data) {
            ...
            const cust = (custData.data || []).find(c => c.id === q.customer_id);
            if (cust) setCustomerAddress(cust.address || '');
            const dbProducts = (q.products || []).map((p, i) => ({
              ...p, key: 'prod_' + p.id, profile_series: null, glass: null, color: null, hardware: null,
            }));
            setProducts(dbProducts);
            const dbFees = (q.fees || []).map((f, i) => ({ ...f, key: 'fee_' + f.id }));
            setFees(dbFees);
            setDiscountAmount(q.discount_amount || 0);
            setDiscountType('fixed');
            setPaymentMethod(q.payment_method || '');
            setDeliveryDays(String(q.delivery_days || 15));
            setWarrantyYears(String(q.warranty_years || 5));
            setValidDays(q.valid_days || 30);
            setRemark(q.remark || '');
            setTimeout(() => calculateTotals(dbProducts, dbFees, q.discount_amount || 0), 0);
          }
        })
        .catch(err => console.error('加载报价数据失败:', err));
    }
  ...
  // 保存确认后的处理
  const handleSaveConfirm = async (asNew) => {
    ...
    try {
      if (asNew) {
        const res = await fetch('/api/quotes', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            source_quote_id: editingQuoteId,
            user_id: 1,
            customer_id: customerId,
            product_total: productTotal,
            fee_total: feeTotal,
        ...
        if (result.success) {
          setSavedQuoteNo(result.data.quote_no);
          message.success('保存为新报价成功！报价编号：' + result.data.quote_no);
          const custRes = await fetch('/api/customers?userId=1');
          const custData = await custRes.json();
          setCustomers(custData.data);
          setTimeout(() => router.push('/quotes'), 500);
    ...
    <div style={{ padding: '16px 20px', minHeight: '100vh', background: '#f5f7fa' }}>
      ...
      <div style={{
        ...
        <Space>
          ...
          <Button icon={<SaveOutlined />} onClick={() => isEditingMode ? setShowSaveModal(true) : handleSubmit(true)}>保存草稿</Button>
          <Button type="primary" icon={<CheckOutlined />} loading={submitting} onClick={() => isEditingMode ? setShowSaveModal(true) : handleSubmit(false)}>保存报价</Button>
        </Space>
      </div>

      {/* 编辑模式提示条 */}
        {isEditingMode && editingQuoteId && (
          <Card style={{ marginBottom: 16, background: '#fffbe6', borderColor: '#ffe58f' }}>
            <Space>
              <Tag color="orange">编辑中</Tag>
              <Text>当前正在编辑：</Text>
              <Text strong>报价编号：{editQuoteNo}</Text>
              <Text type="secondary"> &middot; </Text>
              <Text strong>客户：{editCustomerName}</Text>
            </Space>
          </Card>
        )}
        ...
        >
          <div style={{ padding: '16px 0' }}>
            <div style={{ marginBottom: 16, fontSize: 14 }}>
              <Text type="secondary">当前为历史报价，请选择保存方式：</Text>
            </div>
            <Space direction="vertical" style={{ width: '100%' }} size={12}>
              <Button
                block
                size="large"
                type={saveAsNew ? 'primary' : 'default'}
                onClick={() => setSaveAsNew(true)}
                style={{ textAlign: 'left', justifyContent: 'flex-start' }}
              >
                <div>
                  <Text strong>保存为新报价</Text>
                  <div style={{ fontSize: 12, color: '#999', marginTop: 2 }}>生成新的报价编号，保留原报价</div>
                </div>
              </Button>
              <Button
                block
              ...
              >
                <div>
                  <Text strong>覆盖原报价</Text>
                  <div style={{ fontSize: 12, color: '#999', marginTop: 2 }}>不可恢复，将替换原报价数据</div>
                </div>
              </Button>
            </Space>
            <Divider />
            <Space style={{ justifyContent: 'flex-end' }}>
              ...
              <Button
                type="primary"
                loading={submitting}
                onClick={() => handleSaveConfirm(saveAsNew)}
              >
                {saveAsNew ? '保存为新报价' : '覆盖原报价'}
              </Button>
            </Space>
          </div>
        </Modal>
